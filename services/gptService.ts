...
export class GPTService {
  ...
  private validateQuestionFormat(question: Question): boolean {
    try {
      ...
      if (!question.explanation?.correct?.trim() || !question.explanation?.key_point?.trim())
        return false;
      ...
      if (question.explanation.correct.length < 5 || question.explanation.key_point.length < 5)
